...
def get_config(excel_name, sheet_name):
    ...
    excel_name = os.path.join(os.getcwd(), "input", excel_name)
    try:
...
def generate_qr(config, picture="ye.png"):
    output_dir = os.path.join(os.getcwd(), "二维码")
    picture = os.path.join(os.getcwd(), "input", picture)
    ...
    for port, val in config.items():
        ...
        myqr.run(des_str.decode(), save_name=pic_name, picture=picture, save_dir=output_dir)
